...
tw_redraw() {
  ...
  if [ "$tw_layout" = "half_pure" ]; then
    ...
    if [ "$tw_git_info" != "" ]; then
      if [ "$tw_layout" = "singleline_verbose_alt" ]; then
        tw_git_arrow_info="  (%F{$tw_git_branch_color}$tw_git_info)"
      else
    ...
    if [ "$tw_layout" = "pure" ]; then
      ...
      elif [ "$tw_layout" = "singleline_verbose_alt" ]; then
      ...
      if [ "$tw_layout" = "singleline_verbose_alt" ]; then
        RPROMPT="$tw_right_prompt_prefix$tw_git_arrow_info"
      else
